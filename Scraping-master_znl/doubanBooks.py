...
def crawl():
    # 获取链接
    channel = []
    with open('channel.txt', encoding='utf-8', errors='ignore') as file:
        channel = file.readlines()
    for url in channel:
        data = [] #存放每一本书的数据
        web_data = login(url.strip())
        print(str('web_data') + str(web_data))
        ...
        for book in books:
            ...
            try:
                ...
                path = "covers/"+title+".png"
                urllib.request.urlretrieve(coverUrl, path)
